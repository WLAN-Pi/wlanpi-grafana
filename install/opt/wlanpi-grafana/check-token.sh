...
if [ ${#GRAFANA_TOKEN} -lt 40 ]; then
    ...
    SA_RESPONSE=$(curl -s --insecure -X POST https://wlanpi:wlanpi@127.0.0.1/app/grafana/api/serviceaccounts -H "Accept: application/json" -H "Content-Type: application/json" -d '{"name": "wlanpi", "role": "Admin", "isDisabled": false}' -w " StatusCode:%{http_code}")
    ...
    NEW_GRAFANA_TOKEN=$(curl -s --insecure -X POST https://wlanpi:wlanpi@127.0.0.1/app/grafana/api/serviceaccounts/2/tokens -H "Accept: application/json" -H "Content-Type: application/json" -d '{"name": "wlanpi", "role": "Admin"}' | jq -r '.key')
